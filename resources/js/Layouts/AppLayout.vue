        ...
        <nav class="bg-white border-b border-gray-100">
                ...
                <div class="flex justify-between h-16">
                    <div class="flex">
                        ...
                        <div class="hidden space-x-8 sm:-my-px sm:ml-10 sm:flex">
                            <jet-nav-link  :href="route('term.dashboard')" :active="route().current('term.dashboard')">
                                Terms
                            </jet-nav-link>
                            <jet-nav-link v-if="$page.user.role_id == 1" :href="route('supervisor.dashboard')" :active="route().current('supervisor.dashboard')">
                                Supervisors
                            </jet-nav-link>
                            <jet-nav-link :href="route('internship.dashboard')" :active="route().current('internship.dashboard')">
                                Internships
                            </jet-nav-link>
                        </div>
                    </div>
                    ...
                    <div class="hidden sm:flex sm:items-center sm:ml-6">
                        <div class="relative ml-3">
                            <jet-dropdown align="right" width="48">
                                <template #trigger>
                                    <button v-if="$page.jetstream.managesProfilePhotos" class="flex text-sm transition duration-150 ease-in-out border-2 border-transparent rounded-full focus:outline-none focus:border-gray-300">
                                            ...
                                            <form @submit.prevent="switchToTeam(team)" :key="team.id">
                                                <jet-dropdown-link as="button">
                                                    <div class="flex items-center">
                                                        <svg v-if="team.id == $page.user.current_team_id" class="w-5 h-5 mr-2 text-green-400" fill="none" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" stroke="currentColor" viewBox="0 0 24 24"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                                        <div>{{ team.name }}</div>
                                                    </div>
                                                </jet-dropdown-link>
                                            </form>
            ...
            <div :class="{'block': showingNavigationDropdown, 'hidden': ! showingNavigationDropdown}" class="sm:hidden">
                <div class="pt-2 pb-3 space-y-1">
                    <jet-responsive-nav-link :href="route('term.dashboard')" :active="route().current('term.dashboard')">
                        Terms
                    </jet-responsive-nav-link>
                    <jet-responsive-nav-link v-if="$page.user.role_id == 1" :href="route('supervisor.dashboard')" :active="route().current('supervisor.dashboard')">
                        Supervisors
                    </jet-responsive-nav-link>
                    <jet-responsive-nav-link :href="route('internship.dashboard')" :active="route().current('internship.dashboard')">
                        Internships
                    </jet-responsive-nav-link>
                </div>
